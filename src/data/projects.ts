import { Project } from '../types/project';

export const projects: Project[] = [
 {
    id: 'sistema_gestion_academica',
    title: 'Academic Management System',
    description: 'Python-based system designed for educational institutions to manage academic operations. (Work in progress)',
    technologies: ['Python', 'SQL', 'VSC', 'Git'],
    type: 'python',
    link: 'https://github.com/ignacio-leonel/sistema_gestion_academica',
    image: 'https://images.pexels.com/photos/267885/pexels-photo-267885.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
  }, 
 
  {
    id: 'ventas_china',
    title: 'Supermarket Sales Analysis - China',
    description: 'Data analysis of real supermarket sales in China to identify trends and performance metrics.',
    technologies: ['Python', 'Pandas', 'Jupyter Notebook'],
    type: 'jupyter',
    link:'https://github.com/ignacio-leonel/ventas_china',
    image: 'https://images.pexels.com/photos/1001914/pexels-photo-1001914.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
  },
 
  {
    id: 'yxaworld',
    title: 'YXA World Sales Dashboard',
    description: 'Comprehensive sales analysis and reporting using Power BI for business insights.',
    technologies: ['Power BI', 'Data Visualization'],
    type: 'powerbi',
    link:'https://github.com/ignacio-leonel/yxaworld',
    image: 'https://images.pexels.com/photos/669619/pexels-photo-669619.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
  },
 
  {
    id: 'analisis_de_gdp',
    title: 'GDP Analysis by Country and Continent',
    description: 'Exploratory data analysis of GDP across various countries and continents.',
    technologies: ['Python', 'Pandas', 'Matplotlib'],
    type: 'jupyter',
    link:'https://github.com/ignacio-leonel/analisis_de_gdp',
    image: 'https://images.pexels.com/photos/4386373/pexels-photo-4386373.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
  },
   {
    id: 'analisis_ebay',
    title: 'Ebay Guitar Sales Analysis',
    description: 'Web scraping and data analysis of guitar sales on eBay, focusing on segmented sales data.',
    technologies: ['Python', 'Jupyter Notebook', 'Web Scraping'],
    type: 'jupyter',
    link:'https://github.com/ignacio-leonel/Analisis_Ebay',
    image: 'https://images.pexels.com/photos/63695/pexels-photo-63695.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
  },
];
